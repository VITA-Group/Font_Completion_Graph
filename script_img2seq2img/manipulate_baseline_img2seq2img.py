...
def manipulate_points(input, endpoints):
    ...
    p14 = input[endpoints[14] - 1, :2]
    p0 = input[0, :2]
    p1 = input[endpoints[1] + 1, :2]

    p2 = input[endpoints[2], :2]

    p3 = input[endpoints[3] - 1, :2]
    p5 = input[endpoints[5] + 1, :2]

    l_0_1 = [p0[1] * (1. - 1. / endpoints[1] * n) + p1[1] * 1. / endpoints[1] * n for n in range(endpoints[1])]
    output[0:endpoints[1], 0] = p1[0]
    output[0:endpoints[1], 1] = np.array(l_0_1)
    output[0:endpoints[1], 2:] = np.array([0, -1])

    l_1_2 = [p1[1] * (1. - 1. / endpoints[1] * n) + p2[1] * 1. / endpoints[1] * n for n in range(endpoints[1])]
    output[endpoints[1]:endpoints[2], 0] = p2[0]
    output[endpoints[1]:endpoints[2], 1] = np.array(l_1_2)
    output[endpoints[1]:endpoints[2], 2:] = np.array([0, -1])

    l_2_3 = [p2[1] * (1. - 1. / endpoints[1] * n) + p3[1] * 1. / endpoints[1] * n for n in range(endpoints[1])]
    output[endpoints[2]:endpoints[3], 0] = p1[0]
    output[endpoints[2]:endpoints[3], 1] = np.array(l_2_3)
    output[endpoints[2]:endpoints[3], 2:] = np.array([0, -1])

    l_3_4 = [p3[1] * (1. - 1. / endpoints[1] * n) + p5[1] * 1. / endpoints[1] * n for n in range(endpoints[1])]
    output[endpoints[3]:endpoints[4], 0] = p3[0]
    output[endpoints[3]:endpoints[4], 1] = np.array(l_3_4)
    output[endpoints[3]:endpoints[4], 2:] = np.array([0, -1])

    l_4_5 = [p3[0] * (1. - 1. / endpoints[1] * n) + p5[0] * 1. / endpoints[1] * n for n in range(endpoints[1])]
    output[endpoints[4]:endpoints[5], 0] = np.array(l_4_5)
    output[endpoints[4]:endpoints[5], 1] = p5[1]
    output[endpoints[4]:endpoints[5], 2:] = np.array([-1, 0])

    l_14_15 = [p1[0] * (1. - 1. / endpoints[1] * n) + p14[0] * 1. / endpoints[1] * n for n in range(endpoints[1])]
    output[endpoints[14]:endpoints[15], 0] = np.array(l_14_15)
...
if __name__ == '__main__':
    fname = 'B_ofl_eczar_Eczar-Regular'
    # 'B_ofl_trirong_Trirong-ExtraBoldItalic'
    # 'B_ofl_rhodiumlibre_RhodiumLibre-Regular'
    # 'B_ofl_bitter_static_Bitter-Bold'
    # 'B_ofl_belgrano_Belgrano-Regular'

    forward_only = True

    save_name = '%s_%s' % (fname, 'mannual')
    if forward_only:
